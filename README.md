...
2. Read the regime. It checks fourteen axes:
...
- Circular AI financing: is the AI capex cycle fragile or real? (vendor financing, cross-investment loops, token profitability; lender-grade residuals count only on executed-sale prints, with dated compute-derivative milestones tracked as event watches; EDGAR disclosure counts and the Ornn compute index give the automated floor)
- Crowding: who already holds it, and can they leave? (benchmark top-10 weight, optional 13F drill-down read from the filing itself — execution evidence only, never direction)
Each axis gets a verdict. The five market axes (valuation, complacency, macro, microstructure, prediction) form one market pulse: EXPANSION, LATE CYCLE, CONTRACTION, or CRISIS. Correlation, regime models, and the seven structural/thematic axes (fiscal through crowding) apply as modifiers — defense floor, risk budget, offense ceiling, and an execution ceiling on size and staging. The 2026 regime is "both truths at once": fiscal/currency fragility (defense) and the AI infrastructure/energy supercycle (offense) are simultaneously true, and neither vetoes the other. A bundled tool (`tools/market_pulse.py`) pulls every axis live — no API keys, nothing to install — from free sources (FRED, CBOE, treasury.gov, multpl, stockanalysis, Polymarket, Kraken, SEC EDGAR, Ornn). The remaining qualitative reads (credit-rating trend, the circular-financing judgment call) are flagged MANUAL and go to primary sources: the filing, not the dashboard.
3. Calibrate posture. The pulse meets your goals in a matrix. Late cycle plus a growth goal means trim and raise cash. Late cycle plus a preservation goal means cut equity to the floor. Same pulse, different posture. Income-fund picks (monthly/weekly payers, including newer systematic/AI-marketed option funds) pass a seasoning gate: unseasoned funds enter capped and in thirds regardless of recent outperformance.
...
Everything in `invest-optimizer/tools/` is plain Python 3 standard library. Nothing to pip install, no keys to configure. Every data source fails soft: a missing reading downgrades that axis, and the brief still ships. EDGAR document fetches want a declared contact: `export EDGAR_USER_AGENT="Name you@example.com"`.
- `market_pulse.py` — the full fourteen-axis regime read in one run: Shiller CAPE, Buffett Indicator, S&P/M2, VIX, high-yield spread, the yield curve, a limited SMH high-low-range proxy, Polymarket recession/rate-hike odds, 60-day equity and equity–bond correlations, a thin 2-state HMM check, the structural axes (debt-to-GDP and interest/revenue, DXY proxy, 10y TIPS real yield, gold, M2, Fed balance sheet, BTC vs 200-week MA, SPY 200-day/200-week trend), EDGAR disclosure counts plus the Ornn compute index for the AI-financing axis, and crowding (benchmark top-10 weight; `cik=NNNNNNNNNN` reads a named manager's latest 13F-HR from EDGAR). The proxy is not treated as a reversal, flash-crash, market-wide, or AI-attribution measure.
...
### What you get
A posture brief with the profile, evidence-dated market pulse, confidence and model disagreement, current-to-target allocation actions, reversal gates, exact weights, liquidity/capacity assumptions, and forward-risk results. Stated probabilities are logged in `FORECASTS.md` with Brier scoring and quarterly review; income holdings carry an `INCOME.md` dashboard with distribution/ROC/NAV tripwires. Learned signals include their experiment provenance and promotion status. If stock selection is appropriate, candidates pass explicit technical gates; they remain a research list, not a buy list.

### AI/quant guardrails
The skill borrows useful research patterns from Qlib and tools catalogued by Awesome Quant without assuming any repository creates alpha. Evaluated-only pointers cover agent strategy research (Vibe-Trading), a paper-trading/data terminal (FinceptTerminal), and swarm execution (AutoHedge-class, research-only given approval-gated posture). It favors deterministic baselines, point-in-time data, reproducible experiments, realistic execution, maintained libraries, and the smallest stack that closes a named gap. Deep learning, graph models, transformers, RL, LLM sentiment, synthetic scenarios, and model-zoo results never receive automatic allocation authority.

### The quick version
Ask only about market conditions and the skill skips your portfolio. You get a compact pulse table: valuation, complacency, macro, microstructure, prediction, fiscal, currency, money-creation, digital-asset, secular trend, AI-financing, crowding, overall.
...
- **2026-09-23** — `github - armchairfuturist-code/Skills` (`invest-optimizer/`) — **fourteen regime axes.** New **2N Crowding** reads benchmark top-10 weight (SPY/QQQ from `stockanalysis`) and, with `cik=NNNNNNNNNN`, a named manager's latest 13F-HR parsed straight from EDGAR; it tightens Phase 4 size, staging, and the exit plan and never sets direction. **EDGAR becomes a first-class source**: `feeds.edgar_search` (full-text disclosure counts, no key), `feeds.edgar_filings`, `feeds.edgar_13f` — 13F/insider aggregators are caches over it, so the filing is now the primary source for vendor-claim verification and the 2M circular-financing read (90-day disclosure counts plus the Ornn compute index). **Fixed** a stale `SKILL.md` tool-fallback note claiming `market_pulse.py` covered only 2A–2G, which sent runs into manual web reads for six already-automated axes; the dead-feed list now lives in one place, `tools/feeds.py`, with dated probe verdicts (2026-09-23: Kalshi's keyless API answers but returns no prices, so Polymarket stays the only 2E venue; finviz/macrotrends 403, aggregator sites unusable via plain GET). EDGAR document fetches need `EDGAR_USER_AGENT="Name you@example.com"`.
